...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Text(),
            nullable=False,
            server_default=sa.text("'human_weighted'"),
        ),
        sa.Column(
            "pet_weight", sa.Float(), nullable=False, server_default=sa.text("0.25")
        ),
    ...
    op.create_index(
        "ix_anomaly_episodes_room_start_ts", "anomaly_episodes", ["room", "start_ts"]
    )
