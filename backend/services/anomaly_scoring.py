...
def _observed_activity(db: Session, room: str, start: datetime, end: datetime, *, pet_weight: float, unknown_weight: float) -> tuple[float, dict]:
    """
    ...
    return total, {"episodes_used": used, "pet_weight": pet_weight, "unknown_weight": unknown_weight}


def _observed_door_events(db: Session, room: str, start: datetime, end: datetime) -> int:
...
def score_room_bucket(
    ...
    *,
    ...
    if bucket_start.tzinfo is None:
        raise HTTPException(status_code=400, detail="bucket_start must be timezone-aware UTC")

    bucket_start = bucket_start.astimezone(timezone.utc).replace(second=0, microsecond=0)
    ...
    details: dict[str, Any] = {
        ...
        "model_end": model_end,
        "room": room,
        "bucket": {"start": bucket_start.isoformat(), "end": bucket_end.isoformat(), "bucket_idx": bucket_idx, "dow": dow, "is_weekend": is_weekend},
    }

    # Observed
    activity_obs, act_meta = _observed_activity(db, room, bucket_start, bucket_end, pet_weight=pet_weight, unknown_weight=unknown_weight)
    door_obs = _observed_door_events(db, room, bucket_start, bucket_end)

    details["observed"] = {"activity_obs": activity_obs, "door_obs": door_obs, **act_meta}
    ...
    if not model_end:
        reasons.append({"reason_code": "BASELINE_STATUS_MISSING", "component": "meta", "points": 0.0, "evidence": {"note": "No baseline_model_status rows"}})
        total = 0.0
        return BucketScore(
            room=room, bucket_start=bucket_start, bucket_end=bucket_end,
            dow=dow, is_weekend=is_weekend, bucket_idx=bucket_idx,
            score_total=total, score_intensity=0.0, score_sequence=0.0, score_event=0.0,
            level=_level_from_score(total), reasons=reasons, details=details
        )
    ...
    b = db.execute(
        ...
        {"uid": uid, "model_end": model_end, "dow": dow, "is_weekend": is_weekend, "room": room, "bucket_idx": bucket_idx},
    ).mappings().first()

    if not b:
        reasons.append({"reason_code": "BASELINE_MISSING_ROOM_BUCKET", "component": "meta", "points": 0.0, "evidence": {"room": room, "bucket_idx": bucket_idx, "dow": dow, "is_weekend": is_weekend}})
    else:
        ...
        if mu is None or sig is None or n <= 0:
            reasons.append({"reason_code": "BASELINE_ACTIVITY_UNSUPPORTED", "component": "intensity", "points": 0.0, "evidence": {"support_n": n, "mu": mu, "sigma": sig}})
        else:
            mu = float(mu)
            sig = float(sig)
            ...
            if score_intensity > 0:
                reasons.append({
                    ...
                    "evidence": {"obs": activity_obs, "mu": mu, "sigma_eff": sigma_eff, "z": z, "support_n": n},
                })
        ...
        if dmu is None or dsig is None or dn <= 0:
            ...
            reasons.append({"reason_code": "BASELINE_DOOR_UNSUPPORTED", "component": "event", "points": 0.0, "evidence": {"support_n": dn, "mu": dmu, "sigma": dsig}})
        else:
            dmu = float(dmu)
            dsig = float(dsig)
            ...
            if score_event > 0:
                reasons.append({
                    ...
                    "evidence": {"door_obs": door_obs, "mu": dmu, "sigma_eff": dsigma_eff, "z": dz, "support_n": dn},
                })
    ...
    if prev and prev != room:
        t = db.execute(
            ...
            {"uid": uid, "model_end": model_end, "dow": dow, "is_weekend": is_weekend, "bucket_idx": bucket_idx, "from_room": prev, "to_room": room},
        ).mappings().first()

        if not t or t["p_smoothed"] is None:
            reasons.append({"reason_code": "TRANSITION_BASELINE_MISSING", "component": "sequence", "points": 0.0, "evidence": {"from_room": prev, "to_room": room}})
        else:
            ...
            if score_sequence > 0:
                reasons.append({
                    ...
                    "evidence": {"from_room": prev, "to_room": room, "p": p, "p_floor": p_floor, "rarity": rarity,
                                 "trans_count": float(t.get("trans_count") or 0.0), "from_total": float(t.get("from_total") or 0.0), "alpha": float(t.get("alpha") or 0.0)},
                })
