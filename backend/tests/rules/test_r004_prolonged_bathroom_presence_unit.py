...
def test_r004_day_segment_triggers_medium():
    ...
    devs = eval_r004_prolonged_bathroom_presence(
        _ctx(rows, since, until, now, params={"tz": "UTC"})
    )
    assert len(devs) == 1
...
def test_r004_night_boost_triggers_high():
    ...
    params = {
        "tz": "UTC",
        "night_threshold_min": 25,
        "day_threshold_min": 45,
        "night_start_hour": 22,
        "night_end_hour": 7,
    }
    devs = eval_r004_prolonged_bathroom_presence(
        _ctx(rows, since, until, now, params=params)
    )
    assert len(devs) == 1
