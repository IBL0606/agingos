...
def psql_scalar(sql: str) -> str:
    ...
    p = subprocess.run(cmd, input=sql.encode("utf-8"), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if p.returncode != 0:
        raise RuntimeError(p.stderr.decode("utf-8"))
    return p.stdout.decode("utf-8").strip()

def gdpr_report(org_id: str, home_id: str, subject_id: str, dry_run: bool) -> Dict[str, Any]:
    sql = (
...
def compute_token(org_id: str, home_id: str, subject_id: str, report: Dict[str, Any], salt: str) -> str:
    body = json.dumps(report, sort_keys=True, separators=(",", ":"))
    raw = f"{org_id}|{home_id}|{subject_id}|{salt}|{body}".encode("utf-8")
    return hashlib.sha256(raw).hexdigest()[:16]

def main():
    ap = argparse.ArgumentParser(description="Pilot-safe GDPR delete scope wrapper for AgingOS")
    ...
    ap.add_argument("--confirm-token", default="", help="Required when using --execute. From latest dry-run.")
    ap.add_argument("--vacuum-analyze", action="store_true", help="Run VACUUM(ANALYZE) after execute.")
    ap.add_argument("--reset-token", action="store_true", help="Reset token salt file for this scope (dry-run only).")
    args = ap.parse_args()
    ...
    if not args.confirm_token or args.confirm_token != token:
        raise SystemExit("REFUSING: --execute requires --confirm-token matching the latest dry-run token for this scope.")
