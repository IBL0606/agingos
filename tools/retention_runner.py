...
TABLES = [
    ...
    ("episodes_svc", "retention_preview_episodes_svc", "retention_prune_episodes_svc", 2000),
    ("anomaly_episodes", "retention_preview_anomaly_episodes", "retention_prune_anomaly_episodes", 2000),
...
def prune(scope: Tuple[str,str,str], before: dt.datetime, execute: bool, max_rounds: int) -> Dict[str,int]:
    org_id, home_id, subject_id = scope
...
def main():
    ...
    ap.add_argument("--days", type=int, default=180, help="Retention window in days (delete older than now - days)")
    ap.add_argument("--execute", action="store_true", help="Actually delete. Without this flag it's dry-run.")
    ap.add_argument("--max-rounds", type=int, default=200, help="Max batches per table per scope")
    args = ap.parse_args()
